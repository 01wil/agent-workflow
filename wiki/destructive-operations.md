...
**This document exists because it already went wrong.** An agent once deleted 130 rows from a
production table while the user had explicitly said *"don't change anything until the lead
approves"*. The agent believed `SET PARSEONLY ON` made the statement a syntax check. It did not. No
backup existed. The rows were unrecoverable. The outcome happened to be correct, which was luck, not
competence.
...
This is exactly the trap that caused the incident. The empty result output looked like a clean
syntax check. It was a completed deletion.
...
| `git push --force`, history rewrite | Never without explicit approval. |
...
| Bulk file rename / restructure | Propose the full list of moves, wait for approval. |
...
## Case study: the deduplication incident

**What ran:**
```sql
SET PARSEONLY ON;
;WITH r AS (SELECT ROW_NUMBER() OVER (...) AS rn
            FROM [PRODDB01].reporting.dbo.DailyReport)
DELETE FROM r WHERE rn > 1;
SET PARSEONLY OFF;
```

**Intent:** syntax-check the statement. **Actual effect:** deleted 130 production rows.

**Result:** 14,452 → 14,322 rows. The deleted rows were genuine duplicates and no distinct record
was lost — verified against a baseline measured earlier. Correct outcome, achieved by luck.

**Every failure in sequence:**
1. Used an untested mechanism (`PARSEONLY`) as a safety net.
2. Pointed a `DELETE` at production while merely exploring.
3. Ignored a standing instruction to change nothing.
4. Took no backup, despite having already written a script that creates one.
5. Read empty output as "nothing happened".
6. Had a rule available (`AGENTS.md`: "Do not delete data … unless explicitly authorized") and
   still did it — because the statement did not *feel* like a delete.

**The generalisable lesson:** the danger is not ignorance of the rule. It is believing a
destructive statement has been rendered harmless. **Only methods A, B, and C above are
verified safe. Nothing else counts.**
