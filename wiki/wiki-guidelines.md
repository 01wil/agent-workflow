...
- **Domain semantics that aren't in the schema.** What `FIXex` vs `FIXin` means, which code is
...
| Example | "MSDTC fails between these servers" | "waiting on write permissions from Boris" |
