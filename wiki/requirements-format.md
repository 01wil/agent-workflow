...
code before checkable acceptance criteria. This file owns the artifact that survives the session, so
the contract is not lost at compaction and the next agent inherits the agreed edges and their order.
...
> Position in the pipeline: requirements.md (the contract) then the work then state.md (the record).
> Brainstorm the requirements, do the work, log the result. The requirements' priority order becomes
> the todo order in state.md.
...
Valid:
```markdown
- `SELECT COUNT(*) FROM contracts WHERE dup_flag = 1` returns 0 (was 130), and the daily job still
  produces a report with an unchanged prior-day row count. Checked on the dev database.
```

Not valid (cannot be run):
```markdown
- The duplicate rows are gone and everything still works.
```
