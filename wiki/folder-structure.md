...
There is no plan.md, questions.md, or analysis.md. Execution steps are sub-todos in state.md.
Questions to people are todos with a paste-ready message in state.md. Durable "how it works"
understanding is a shipped deliverable (ARCHITECTURE.md, src-docs.md). Two files to resume from,
not five — deliberately.
...
notes/ is a sibling folder, not a child, so it needs no ignore entry. The src/ repo still wants the
usual exclusions:
...
4. If the folder is a flat pile of received files, propose the split — list what moves where — and
   wait for approval. Do not move anything unasked.
