...
5. **Concrete numbers, always.** "130 excess rows out of 14,452," not "some duplicates." Give the
   exact identifier the reader can look up (an order number, an insert date).
...
8. **No AI tells.** Avoid "I hope this helps," "happy to," "let me know," "in summary," "it's
   important to note," em-dashes as dramatic pauses, and any sentence that exists only to sound
   polite.
9. **Own the uncertainty.** Hedge where hedging is honest ("that's fairly arbitrary," "a bit ugly,"
   "maybe a starting point"), be blunt where it is not.
10. **Match the language and register the project already uses.** Internal notes and code default to
