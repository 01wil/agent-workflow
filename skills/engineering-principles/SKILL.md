...
In this environment that usually means PowerShell via `pwsh.exe`, a `SELECT` that counts what a
`DELETE` would touch, or a `sqlcmd` script committed next to the change. See
`knowledge/powershell-windows-access.md`.
...
- **Brand semantic primitives.** `ContractId` and `OrderId` are both `int` underneath and must not be
  interchangeable. A readonly record struct wrapping the value costs little and turns an argument
  transposition into a compile error. Validate once at creation, then trust the type.
