...
- **One to three statements.** If a point needs six queries, the point was not understood. Combine:
  ...
  six that narrate it.
- **Fragment comments, not prose.** `-- 215590: zwei zeilen, nur der trader ist anders` — not
  `-- Dieser Block zeigt, dass die beiden Zeilen identisch sind bis auf den exectrader.` No
  full sentences, no capitals where the user writes lowercase, no trailing periods.
- **No structural ceremony.** No `-- 1)` numbering, no `=====` separators, no "alles nur select,
  nichts wird geaendert" reassurance, no block explaining how to run it. Two blank lines between
  statements is the whole structure.
...
- **Leave the loose end visible.** A real note reads `-- 103716 ist von 2023, steht in einer
  _until_-tabelle`, sometimes with a `???`. An agent smooths that into a paragraph or, worse,
  writes code to handle it. Keep the note, drop the handling.
